    ...
    if (on_menu) {
        ...
        for (uint8_t row = 0; row < LCD_ROWS; row++) { //main-menu print
            ...
            switch (current_text_row) {
                case DEC_: {
                    lcd.print(menu_str[DEC_]);
                    lcd.print(" : ");
                    lcd.print(menu_op_value[DEC_] / 60);
                    lcd.write(byte(223));
                    lcd.print(menu_op_value[DEC_] % 60);
                    lcd.print("'   ");
                    if (auto_mode) {
                    ...
                    break;
                case RA: {
                    lcd.print(menu_str[RA]);
                    lcd.print(" : ");
                    lcd.print(menu_op_value[RA] / 60);
                    lcd.print("h");
                    lcd.print(menu_op_value[RA] % 60);
                    lcd.print("m   ");
                    if (auto_mode) {
                    ...
                    break;
                case Hemisphere: {
                    lcd.print(menu_str[Hemisphere]);
                    lcd.print(": ");
                    menu_op_value[Hemisphere] ? lcd.print("N    ") : lcd.print("S    ");
                }
                    break;
                case tempo_tela: {
                    lcd.print(menu_str[tempo_tela]);
                    lcd.print("    ");
                    lcd.print(menu_op_value[tempo_tela]);
                    lcd.print("s     ");
                }
                    break;
                default: {
                    lcd.print(menu_str[current_text_row]);
                    lcd.print(" : ");
                    lcd.print(menu_op_value[current_text_row]);
                    lcd.print("        "); //linha print
                }
            }
        }
        ...
        lcd.setCursor(0, 0);
        lcd.print(" ");
        lcd.print(menu_str[current_selection]);
        lcd.print("           "); //titulo submenu
        ...
        switch (current_selection) {
            ...
            case RA: { //right ascendence
                if (auto_mode)
                    (value_preview == menu_op_value[1]) ?: value_preview = menu_op_value[1]; //lock on auto ra
                lcd.setCursor(0, LCD_ROWS - 1);
                lcd.write(byte(1));
                lcd.print("     ");
                lcd.print(value_preview / 60);
            ...
            case Hemisphere: { //hemisphere
                ...
                lcd.write(byte(1));
                lcd.print("     ");
                value_preview ? lcd.print("Norte         ") : lcd.print(" Sul        ");
            }
                break;
            case automatic_mode: { //auto-mode
                ...
                lcd.write(byte(1));
                lcd.print("      ");
                value_preview ? lcd.print("ON        ") : lcd.print("OFF       ");
            }
                break;
            case brilho_tela: { //brightness
                ...
                lcd.write(byte(1));
                lcd.print("      ");
                lcd.print(value_preview);
                lcd.print("%        "); //linha 2 print
            }
                break;
        ...
        if (digitalRead(SELECT) && (current_selection != save_configs)) { //press sem ser no save
            ...
            lcd.setCursor(0, LCD_ROWS - 1);
            lcd.write(byte(2)); //da o simbolo de ok;
            ...
            else auto_mode = false; //inicia o modo automatico
        }
