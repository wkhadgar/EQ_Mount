...
void nRF24_PrepareData(const void* in_data, size_t in_data_len, transf_t transfer_mode, nrf24_data_t* out_data) {
	out_data->kind = transfer_mode;
	out_data->data_size = in_data_len;
	
	if (in_data_len <= NRF24_TRANSF_MAX_LEN) {
		for (uint8_t i = 0; i < NRF24_TRANSF_MAX_LEN; i++) {
			out_data->data[i] = (i <= in_data_len) ? *((uint8_t*) in_data) : 0;
		}
	}
}

void nRF24_RetrieveData(const nrf24_data_t* in_data, transf_t* transfer_mode, void* out_data, size_t* out_data_len) {
	*transfer_mode = in_data->kind;
	*out_data_len = in_data->data_size;
	
	if ((*out_data_len > 0) && (*out_data_len <= NRF24_TRANSF_MAX_LEN)) {
		for (uint8_t i = 0; i < in_data->data_size; i++) {
